...
def cloud(surface_cloud, color_cloud, x_cloud, y_cloud, h_x_cloud=220,
          h_y_cloud=50
          ):
    '''
    Функция рисует облако
    surface_cloud - объект pygame.Surface
    color_cloud - цвет облака, заданный в формате, подходящем для pygame.Color
    x_cloud - координата по оси x самой левой точки облака
    y_cloud - координата по оси y самой верхней точки облака
    h_x_cloud - длина одного облака (по умолчанию 220)
    h_y_cloud - высота одного облака(по умолчанию 50)
    '''
    pygame.draw.ellipse(surface_cloud, color_cloud,
                        (x_cloud, y_cloud, h_x_cloud, h_y_cloud))    
...
for _ in range(number_of_clouds):
    a = randint(100, 230)
    color_of_cloud = (a, a, a)
    x_of_cloud = randint(-220, 600)
    y_of_cloud = randint(0, 250)
    cloud(screen, color_of_cloud, x_of_cloud, y_of_cloud)
ufo(screen, 40, 300, 1, color_of_ufo_light,
    color_of_ufo_glass, color_of_ufo_korpus, color_of_ufo_fonar, 100)
alien(screen, 352, 261, 1, color_of_alien)
